...
local-deploy:
	...
	aws --endpoint-url http://localhost:4566 lambda create-function --function-name fileevent --handler main --runtime go1.x --role your-role --zip-file fileb://main.zip --environment 'Variables={KAFKA_BROKER=redpanda:29092,SCHEMA_REGISTRY=http://redpanda:8081,TOPIC_NAME=S3FileCreatedUpdated,OPERATIONS="{\"organization\":\"organization_operation\"}"}' && \
